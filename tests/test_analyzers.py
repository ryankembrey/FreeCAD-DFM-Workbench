...
from freecad.DFM.core.utils.geometry import FaceIndex, EdgeIndex
...
class TestSphereThicknessAnalyzer(unittest.TestCase):
    def setUp(self):
        ...
        # face index
        self.cube_fi, self.cube_ei = FaceIndex(self.cube), EdgeIndex(self.cube)
        self.plate_fi, self.plate_ei = FaceIndex(self.plate), EdgeIndex(self.plate)
        self.cylinder_fi, self.cylinder_ei = FaceIndex(self.cylinder), EdgeIndex(self.cylinder)
        self.hole_fi, self.hole_ei = FaceIndex(self.box_with_hole), EdgeIndex(self.box_with_hole)

    def test_cube_max_thickness(self):
        data = self.analyzer.execute(self.cube, self.cube_fi, self.cube_ei)
    ...
    def test_cylinder_bounded_by_diameter(self):
        data = self.analyzer.execute(self.cylinder, self.cylinder_fi, self.cylinder_ei)
        max_overall = max(max(v) for v in data.values() if v)
        self.assertAlmostEqual(max_overall, 30.0, places=1)

    def test_box_with_hole_max_corner_thickness(self):
        data = self.analyzer.execute(self.box_with_hole, self.hole_fi, self.hole_ei)
        max_overall = max(max(v) for v in data.values() if v)
